...
public class Headers {
	...
	static Logger _log = Logger.getLogger(Headers.class.getName());
	...
	public static void processHeaders(URI uri, int status, Header[] headerFields, Callback cb) {
		...
		Resource ruri;
		try {
			// decoding: rebuild URI with only scheme, authority, path, query, fragment
		...
		// Replace deprecated NumericLiteral with a typed Literal for integers using a Resource for the datatype.
		cb.processStatement(new Node[] { bNode,
				new Resource(httpNS + "responseCode"),
				new Literal(String.valueOf(status), null, new Resource("http://www.w3.org/2001/XMLSchema#integer")),
				ruri });
		...
		for (Header headerField : headerFields) {
			if (HEADER_MAP.containsKey(headerField.getName())) {
				...
				if (predicate.equals(HEADER_MAP.get("Content-Location"))) {
					value = new Resource(NxUtil.escapeForNx(uri.resolve(headerField.getValue()).toString()));
				} else {
					value = new Literal(NxUtil.escapeForNx(headerField.getValue()));
				}
